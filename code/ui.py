...
class UI:
    ...
    def quad_select(self, index, options):
        # bg
        rect = pygame.FRect(self.left + 40, self.top - 20, 400, 200)
    ...
    def stats(self):
        # bg
        rect = pygame.FRect(self.left, self.top - 80, 250, 80)
        pygame.draw.rect(self.display_surface, COLORS['white'], rect, 0, 4)
        pygame.draw.rect(self.display_surface, COLORS['gray'], rect, 4, 4)
        ...
        # health bar
        health_rect = pygame.FRect(rect.left + rect.width * 0.05, rect.top + rect.height * 0.6, rect.width * 0.9, rect.height * 0.3)
...
class OpponentUI:
    ...
    def draw(self):
        ...
        # name
        name_surf = self.font.render(self.monster.name, True, COLORS['black'])
        name_rect = name_surf.get_frect(topleft = rect.topleft + pygame.Vector2(rect.width * .05, 12))
        self.display_surface.blit(name_surf, name_rect)
